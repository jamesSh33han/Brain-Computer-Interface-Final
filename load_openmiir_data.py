...
def get_eeg_epochs(fif_file, raw_eeg_data, end_time, fs):
    eeg_epochs = np.array([])
    events = mne.find_events(fif_file)
    events = events[np.logical_not(np.logical_and(events[:,2] > 20, events[:,2] > 999))]
    event_stimulus_ids = []
    for event_index in range(len(events)):
        stimulus_id = events[event_index, 2]//10
    ...
    for start_time in event_start_times:
        ...
        end_epoch = int(int(start_time) + (end_time*fs))
        epoch_data = raw_eeg_data[:, start_epoch:end_epoch]
        eeg_epochs = np.append(eeg_epochs, epoch_data)
    eeg_epochs = np.reshape(eeg_epochs, [len(events), np.size(raw_eeg_data, axis=0), int(end_time*fs)])
    return eeg_epochs, events, event_stimulus_ids

eeg_epochs, events, event_stimulus_ids = get_eeg_epochs(fif_file, raw_eeg_data, end_time, fs)
...
def plot_power_spectrum(eeg_epochs_fft, fft_frequencies, is_trial_greater_than_170bpm, channels_to_plot, channels):
    ...
    # Convert to decibels
    power_in_db_12Hz = 10*np.log10(mean_power_spectrum_g170_norm)
    power_in_db_15Hz = 10*np.log10(mean_power_spectrum_l170_norm)
    ...
    for channel_index, channel in enumerate(channels_to_plot):
        ...
        plt.plot(fft_frequencies,power_in_db_12Hz[index_to_plot], label='> 170 BPM', color='red')
        plt.plot(fft_frequencies,power_in_db_15Hz[index_to_plot], label='< 170 BPM', color='green')
        plt.legend()
...
plot_power_spectrum(eeg_epochs_fft, fft_frequencies, is_trial_greater_than_170bpm, channels_to_plot, channel_names)




# def extract_features(eeg_epochs_fft, fft_frequencies):
